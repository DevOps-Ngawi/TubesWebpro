...
describe('SoalEsai Model', () => {
  ...
  test('createSoalEsai() - tanpa kata kunci (undefined -> null)', async () => {
    const fakeData = { id: 6, id_level: 1, tipe: 'esai', kata_kunci: null };
    prisma.soals.create.mockResolvedValue(fakeData);
  ...
  test('updateSoalEsai() - semua field terisi', async () => {
    ...
    expect(prisma.soals.update).toHaveBeenCalledWith({
      ...
      data: {
        id_level: 2,
        text_soal: 'Soal diperbarui',
        kata_kunci: 'kunci baru'
      }
    });
    expect(result).toEqual(fakeData);
  });

  test('updateSoalEsai() - id_level null menjadi undefined', async () => {
  ...
  test('updateSoalEsai() - text_soal dan kata_kunci undefined', async () => {
    ...
    expect(prisma.soals.update).toHaveBeenCalledWith({
      ...
      data: {
        id_level: 2,
        text_soal: undefined,
        kata_kunci: undefined
      }
    });
    expect(result).toEqual(fakeData);
  });

  test('deleteSoalEsai() - berhasil', async () => {
